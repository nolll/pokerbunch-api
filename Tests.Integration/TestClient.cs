...
public static class TestClient
{
    public static class Action
    {
        public static async Task<TestClientResult> Add(string? token, string cashgameId, AddCashgameActionPostModel parameters) => 
            await PostAsync(token, new ApiActionAddUrl(cashgameId), parameters);

        public static async Task<TestClientResult> Delete(string? token, string cashgameId, string actionId) => 
            await DeleteAsync(token, new ApiActionUpdateUrl(cashgameId, actionId));
        ...
        public static async Task<TestClientResult> Update(string? token, string cashgameId, string actionId, UpdateActionPostModel parameters) => 
            await PutAsync(token, new ApiActionUpdateUrl(cashgameId, actionId), parameters);
    }
    ...
    public static class Bunch
    {
        public static async Task<TestClientResult<BunchModel>> Add(string? token, AddBunchPostModel parameters) => 
            await PostAsync<BunchModel>(token, new ApiBunchAddUrl(), parameters);
        ...
        public static async Task<TestClientResult<IEnumerable<BunchModel>>> ListForUser(string? token) => 
            await GetAsync<IEnumerable<BunchModel>>(token, new ApiUserBunchesUrl());
    ...
    public static class Cashgame
    {
        public static async Task<TestClientResult<CashgameDetailsModel>> Add(string? token, string bunchId, AddCashgamePostModel parameters) => 
            await PostAsync<CashgameDetailsModel>(token, new ApiCashgameAddUrl(bunchId), parameters);

        public static async Task<TestClientResult<IEnumerable<ApiCurrentGame>>> Current(string? token, string bunchId) => 
            await GetAsync<IEnumerable<ApiCurrentGame>>(token, new ApiBunchCashgamesCurrentUrl(bunchId));

        public static async Task<TestClientResult> Delete(string? token, string cashgameId) => 
            await DeleteAsync(token, new ApiCashgameDeleteUrl(cashgameId));

        public static async Task<TestClientResult<CashgameDetailsModel>> Get(string? token, string cashgameId) => 
            await GetAsync<CashgameDetailsModel>(token, new ApiCashgameUrl(cashgameId));
        ...
        public static async Task<TestClientResult<IEnumerable<CashgameListItemModel>>> ListByBunch(string? token, string bunchId, int? year = null) => 
            await GetAsync<IEnumerable<CashgameListItemModel>>(token, new ApiBunchCashgamesUrl(bunchId, year));

        public static async Task<TestClientResult<IEnumerable<CashgameListItemModel>>> ListByEvent(string? token, string eventId) => 
            await GetAsync<IEnumerable<CashgameListItemModel>>(token, new ApiEventCashgamesUrl(eventId));

        public static async Task<TestClientResult<IEnumerable<CashgameListItemModel>>> ListByPlayer(string? token, string playerId) => 
            await GetAsync<IEnumerable<CashgameListItemModel>>(token, new ApiPlayerCashgamesUrl(playerId));
        ...
        public static async Task<TestClientResult<CashgameDetailsModel>> Update(string? token, string cashgameId, UpdateCashgamePostModel parameters) => 
            await PutAsync<CashgameDetailsModel>(token, new ApiCashgameUpdateUrl(cashgameId), parameters);
    }

    public static class Event
    {
        public static async Task<TestClientResult<EventModel>> Add(string? token, string bunchId, EventAddPostModel parameters) => 
            await PostAsync<EventModel>(token, new ApiEventAddUrl(bunchId), parameters);

        public static async Task<TestClientResult<EventModel>> Get(string? token, string eventId) => 
            await GetAsync<EventModel>(token, new ApiEventUrl(eventId));

        public static async Task<TestClientResult<List<EventModel>>> List(string? token, string bunchId) => 
            await GetAsync<List<EventModel>>(token, new ApiEventListUrl(bunchId));
    }

    public static class General
    {
        public static async Task<TestClientResult> ClearCache(string? token) => await PostAsync(token, new ApiAdminClearCacheUrl());
        public static async Task<TestClientResult> Root() => await GetAsync(new ApiRootUrl(), false);
        public static async Task<TestClientResult> Swagger() => await GetAsync(new ApiSwaggerUrl());
        public static async Task<TestClientResult> TestEmail(string? token) => await PostAsync(token, new ApiAdminSendEmailUrl());
        public static async Task<TestClientResult<VersionModel>> Version() => await GetAsync<VersionModel>(new ApiVersionUrl());
    }
    ...
    {
        public static async Task<TestClientResult> Add(AddUserPostModel parameters) => await PostAsync(new ApiUserAddUrl(), parameters);
        ...
        public static async Task<TestClientResult<FullUserModel>> Update(string? token, string userName, UpdateUserPostModel parameters) => 
            await PutAsync<FullUserModel>(token, new ApiUserUpdateUrl(userName), parameters);
    }
    
    private static async Task<TestClientResult> GetAsync(ApiUrl url, bool followRedirect = true) => await GetAsync(null, url, followRedirect);
    ...
    private static async Task<TestClientResult<T>> GetAsync<T>(ApiUrl url) where T : class => await GetAsync<T>(null, url);
    ...
    private static async Task<TestClientResult> PostAsync(ApiUrl url, object parameters) => await PostAsync(null, url, parameters);
    ...
    private static HttpClient GetClient(string? token = null, bool followRedirect = true) => TestSetup.GetClient(token, followRedirect);
