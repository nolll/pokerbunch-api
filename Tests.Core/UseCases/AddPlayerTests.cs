...
class AddPlayerTests : TestBase
{
    ...
    {
        var request = new AddPlayer.Request(new PrincipalInTest(canAddPlayer: true), TestData.SlugA, UniqueName);
    ...
    {
        var request = new AddPlayer.Request(new PrincipalInTest(canAddPlayer: true), TestData.SlugA, EmptyName);
        var result = await Sut.Execute(request);
    ...
    {
        var request = new AddPlayer.Request(new PrincipalInTest(canAddPlayer: true), TestData.SlugA, ExistingName);
        var result = await Sut.Execute(request);
