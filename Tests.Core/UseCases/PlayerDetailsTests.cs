...
class PlayerDetailsTests : TestBase
{
    ...
    {
        var result = await Sut.Execute(CreateRequest(new AccessControlInTest(canSeePlayer: true), TestData.PlayerIdA));
    ...
    {
        var result = await Sut.Execute(CreateRequest(new AccessControlInTest(canSeePlayer: true), TestData.PlayerIdD));
    ...
    {
        var result = await Sut.Execute(CreateRequest(new AccessControlInTest(canSeePlayer: true, canDeletePlayer: false), TestData.PlayerIdA));
    ...
    {
        var result = await Sut.Execute(CreateRequest(new AccessControlInTest(canSeePlayer: true, canDeletePlayer: true), TestData.PlayerIdD));
    ...
    {
        var result = await Sut.Execute(CreateRequest(new AccessControlInTest(canSeePlayer: true, canDeletePlayer: true), TestData.PlayerIdA));

        Assert.That(result.Data?.CanDelete, Is.False);
    }

    private static GetPlayer.Request CreateRequest(IAccessControl accessControl, string playerId)
    {
        return new GetPlayer.Request(accessControl, playerId);
    }
