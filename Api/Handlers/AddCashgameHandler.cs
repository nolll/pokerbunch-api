...
public static class AddCashgameHandler
{
    ...
    {
        var addRequest = new AddCashgame.Request(auth.Principal, bunchId, post.LocationId);
        var addResult = await addCashgame.Execute(addRequest);
        if (!addResult.Success)
            return ResultHandler.Error(addResult.Error);

        var detailsRequest = new CashgameDetails.Request(auth.Principal, addResult.Data?.CashgameId ?? "", DateTime.UtcNow);
