...
{
    public static async Task<IResult> Handle(Buyin buyin, Report report, Cashout cashout, IAuth auth, string cashgameId, [FromBody] AddCashgameActionPostModel post)
    {
    ...
    private static async Task<IResult> Buyin(Buyin buyin, IAuth auth, string cashgameId, AddCashgameActionPostModel post)
    {
    ...
    private static async Task<IResult> Report(Report report, IAuth auth, string cashgameId, AddCashgameActionPostModel post)
    {
        var result = await report.Execute(new Report.Request(auth.Principal, cashgameId, post.PlayerId, post.Stack, DateTime.UtcNow));
        return ResultHandler.Model(result, () => new OkModel());
    }

    private static async Task<IResult> Cashout(Cashout cashout, IAuth auth, string cashgameId, AddCashgameActionPostModel post)
    {
