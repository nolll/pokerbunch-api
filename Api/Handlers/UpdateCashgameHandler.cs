...
public static class UpdateCashgameHandler
{
    ...
    {
        var updateRequest = new EditCashgame.Request(auth.Principal, cashgameId, post.LocationId, post.EventId);
        var updateResult = await editCashgame.Execute(updateRequest);
        if(!updateResult.Success)
            return ResultHandler.Error(updateResult.Error);

        var detailsRequest = new CashgameDetails.Request(auth.Principal, cashgameId, DateTime.UtcNow);
